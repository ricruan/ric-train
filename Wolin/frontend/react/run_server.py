...
React 前端开发服务启动脚本
用法:
    python run_server.py                  # 默认端口 3001，绑定 0.0.0.0
...
def main(
    ...
    if not any(v is not None for v in [port, host, strict_port, build, preview, clear, kill_all]):
        parser = argparse.ArgumentParser(
            description="React 前端开发服务管理脚本",
            formatter_class=argparse.RawDescriptionHelpFormatter,
            epilog="""
示例:
  python run_server.py                  默认端口启动 (0.0.0.0)
  ...
  python run_server.py --kill-all       杀掉所有 node 进程
        ...
        parser.add_argument("--port", "-p", type=int, default=3001, help="服务端口号 (默认: 3001)")
    ...
    p_port = port if port is not None else (args.port if args else 3001)
